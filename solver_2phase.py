...
def shooting(pp: PlanningProblem_alpha, c0, k0, T, λTN1=0.01):
    ...
    if c0 > pp.f(k0) + (1 - pp.δ) * k0 or c0 <= 0:  # 增加 c0 > 0 的检查
        ...
        return nan_vec_T1, nan_vec_T1, nan_vec_T1, nan_vec_T2, nan_vec_T2, nan_vec_T2, nan_vec_T2, False  # 添加一个失败标志
    ...
    I_vec[0] = y_vec[0] - c_vec[0] - Ir_vec[0] - Ic_vec[0] - If_vec[0]

    for t in range(T):
        y_next, c_next, I_next, Ir_next, Ic_next, If_next, k_next = pp.next_y_c_I_Ir_Ic_If_k(c_vec[t], k_vec[t], λTN1)
        if not np.isfinite(k_next) or k_next < 0:  # 增加对路径中资本的检查
            nan_vec_T1 = np.full(T + 1, np.nan)
            nan_vec_T2 = np.full(T + 2, np.nan)
            return nan_vec_T1, nan_vec_T1, nan_vec_T1, nan_vec_T2, nan_vec_T2, nan_vec_T2, nan_vec_T2, False

        y_vec[t + 1], c_vec[t + 1], I_vec[t + 1], Ir_vec[t + 1], Ic_vec[t + 1], If_vec[t + 1], k_vec[t + 1] = y_next, c_next, I_next, Ir_next, Ic_next, If_next, k_next
    ...
    return y_vec, c_vec, I_vec, k_vec, Ir_vec, Ic_vec, If_vec, True  # 添加一个成功标志
...
def bisection(pp: PlanningProblem_alpha, c0, k0, T, λTN1, k_ter, tol=1e-3, max_iter=500):
    ...
    for i in range(max_iter):
        ...
        # 使用布尔标志来判断成功与否
        y_vec, c_vec, I_vec, k_vec, Ir_vec, Ic_vec, If_vec, success = res
        ...
        if np.abs(error) < tol:
            return y_vec, c_vec, I_vec, k_vec, Ir_vec, Ic_vec, If_vec, True
...
def solve_model(pp: PlanningProblem_alpha, c0_guess, k0, λTN1_guess, T, tol=1e-6, max_iter=500):
    """
    ...
    for i in range(max_iter):
        multiplier = pp.β / (1 - pp.β)
        if i==0:
            c_adj = c0_guess
        else:
            c_T = c_vec[-1]
            c_adj = c_T / (1 + λTN1 *pp.η*c_T)
        k_ter = multiplier * c_adj
